export var RtcPeerStates;
(function (RtcPeerStates) {
    RtcPeerStates[RtcPeerStates["Invited"] = 0] = "Invited";
    RtcPeerStates[RtcPeerStates["Answering"] = 1] = "Answering";
    RtcPeerStates[RtcPeerStates["Connected"] = 2] = "Connected";
    RtcPeerStates[RtcPeerStates["Disposed"] = 3] = "Disposed";
})(RtcPeerStates || (RtcPeerStates = {}));
export class RtcPeer {
    constructor(peerId, ourId) {
    ...
    dispose() {
        if (this.state == RtcPeerStates.Disposed)
            return;
    ...
    init() {
        ...
        p.addEventListener(`connectionstatechange`, (evt) => {
    ...
    async createInvite() {
        ...
        if (peer === undefined)
            throw new Error(`Cannot invite while we are not initialised`);
    ...
    async accept(invite) {
        if (this.state == RtcPeerStates.Disposed)
            throw new Error(`Cannot accept whilst disposed`);
        if (this.state == RtcPeerStates.Connected)
            throw new Error(`Cannot accept whilst connected`);
        const p = this.peer;
        if (p === undefined)
            throw new Error(`Cannot accept while not initialised`);
