...
export class ExpiringMultiMap<K, V> {
  private _store:Map<K, Array<ExpiringValue<V>>>;

  constructor(readonly expiryMs:number) {
    this._store = new Map();
    setTimeout(()=> {
      this.maintain();
  ...
  *entries() {
  ...
  dump() {
    ...
    const until = (v:number) => {
      let e = v - now;
      if (e < 1000) return `${e}ms`;
      e /= 1000;
      if (e < 1000) return `${Math.round(e)}s`;
      e /= 60;
...
export class ExpiringMap<K, V>  {
  private _store:Map<K, ExpiringValue<V>>;
